const ATM = {
    ...
    auth: function (number, pin) {
        if (this.is_auth) {
            this.printMessage(
                ...
                this.ACTION_MESSAGE['already_auth'],
                "green");
            return;
        }
        ...
        if (!/\d{4}/.test(number) || !/\d{3}/.test(pin)) {
            this.printMessage(
                "not authorized",
                this.ACTION_MESSAGE['unsucc_auth'],
                "red");
            return;
        }
    ...
    check: function () {
        if (!this.is_auth) {
            this.printMessage(
                "not authorized",
                "attempt " + this.ACTION_MESSAGE['check'] + ", " + this.ACTION_MESSAGE['permission_denied'],
                "red");
            return;
        }
        ...
        this.printMessage(
            this.current_user.number,
            "debit: " + this.current_user.debit,
            "green");
    },

    // get cash - available for user only
    getCash: function (amount) {
        const ACTION = this.ACTION_MESSAGE['withdrawal'] + " " + this.getAmountMessage(amount);
        ...
        if (!this.isEnoughDebit(amount)) {
            this.printMessage(
                this.current_user.number,
                this.ACTION_MESSAGE['insufficient_debit'],
                "orange");
            return;
        }
        ...
        if (!this.isEnoughCash(amount)) {
            this.printMessage(
                this.current_user.number,
                this.ACTION_MESSAGE['not_enough_cash'],
                "orange");
            return;
        }

        this.current_user.debit -= amount;
        this.cash -= amount;
        this.printMessage(
            this.current_user.number,
            ACTION,
    ...
    loadCash: function (amount) {
        const ACTION = this.ACTION_MESSAGE['top_up_account'] + " " + this.getAmountMessage(amount);
        ...
        this.printMessage(
            this.current_user.number,
            this.ACTION_MESSAGE['top_up_account'] + ", current debit: " + this.current_user.debit,
            "green");
    },
    ...
    load_cash: function (addition) {
        ...
        this.printMessage(
            this.current_user.number,
            ACTION + ", current ATM cash: " + this.cash,
            "green");
    },
    ...
    getReport: function () {
        ...
        this.printMessage(
            this.is_auth ? this.current_user.number : "not authorized",
            "attempt read ATM report, " + this.ACTION_MESSAGE['permission_denied'],
            "red");
    },
    ...
    isEnoughDebit: function (amount) {
        return this.current_user.debit - amount > 0;
    },
    ...
    validateAccess: function (action, compareFunc, amount) {
        if (!this.is_auth) {
            this.printMessage(
                "not authorized",
                "attempt " + action + ", " + this.ACTION_MESSAGE['permission_denied'],
                "red");
            return false;
        }
    ...
    getAmountMessage(intStr) {
        return intStr ? intStr + "" : "\u003Cinvalid input\u003E"
    }
};
ATM.init();
